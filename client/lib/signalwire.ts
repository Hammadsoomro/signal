...
export class SignalWireClient {
  ...
  constructor() {
    ...
    this.auth = btoa(
      `${SIGNALWIRE_CONFIG.projectId}:${SIGNALWIRE_CONFIG.token}`,
    );
  }
  ...
  async sendSMS(from: string, to: string, body: string) {
    try {
      ...
      try {
        ...
        const isOwnedNumber = ownedNumbers.incoming_phone_numbers?.some(
          (num: any) => cleanPhoneNumber(num.phone_number) === cleanFrom,
        );

        if (!isOwnedNumber) {
          throw new Error(
            `Phone number ${cleanFrom} is not owned by this SignalWire account. Please purchase this number first or verify it in your SignalWire dashboard.`,
          );
        }
      } catch (validateError) {
      ...
      const response = await fetch(
        `${this.baseUrl}/Accounts/${SIGNALWIRE_CONFIG.projectId}/Messages.json`,
        {
          method: "POST",
          headers: {
  ...
  async purchasePhoneNumber(phoneNumber: string) {
    try {
      const response = await fetch(
        `${this.baseUrl}/Accounts/${SIGNALWIRE_CONFIG.projectId}/IncomingPhoneNumbers.json`,
        {
          method: "POST",
          headers: {
            Authorization: `Basic ${this.auth}`,
  ...
  async getMessages(limit = 20) {
    try {
      const response = await fetch(
        `${this.baseUrl}/Accounts/${SIGNALWIRE_CONFIG.projectId}/Messages.json?PageSize=${limit}`,
        {
          headers: {
            Authorization: `Basic ${this.auth}`,
          },
  ...
  async testConnection() {
    try {
      ...
      const account = await fetch(
        `${this.baseUrl}/Accounts/${SIGNALWIRE_CONFIG.projectId}.json`,
        {
          headers: {
            Authorization: `Basic ${this.auth}`,
          },
      ...
      console.log(
        "Owned phone numbers:",
        numbers.incoming_phone_numbers?.map((num: any) => num.phone_number) ||
          [],
      );
